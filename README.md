...
```java
final InvokeResponse rsp = Spartan.invokeCommand("ETL", "-run-forever");
```
...
Spartan has been in production use for a year, but its development began back in 2015. It originated when C++11 and Java 8 were new. Now C++17 and Java 9 are the latest. In the Java community, the Jetbrains Kotlin language has been making an impact (with quite the boost from Google's adoption of Kotlin for Android programming).

- For the time being the `spartan` C++11 code base will remain compile-able in g++ 4.8.2 as that has been sufficient for its functionality.
...
- The Java Open JDK has **not** been tried with `spartan` - only the Oracle Java JDK (day job has only required Oracle JDK support).
- Java 8 is the only Java language version supported so far. Yet Java 9 is here now and will definitely be important to support. That will require a new round of significant development and testing effort.
...
- Addition of a new enhanced API for invoking worker child process sub commands - perhaps something that returns an extended `InvokeResponse` object:
...
class InvokeResponseEx extends InvokeResponse {
  public final java.io.InputStream ctrlOutStream;
  public final java.io.OutputStream ctrlInStream;
  public InvokeResponseEx(int childPID, java.io.InputStream rspStream, java.io.InputStream  ctrlOutStream, java.io.OutputStream ctrlInStream) {
    super(childPID, rspStream);
    this.ctrlOutStream = ctrlOutStream;
    this.ctrlInStream = ctrlInStream;
  }
}
...
The idea is to allow for an `OutputStream` (the field `ctrlInStream`) that the sub command invoker can write to so as to communicate with its invoked child process. The child process sees the `OutputStream` field `ctrlInStream` as the other end of a pipe stream where it is passed in as an `InputStream` argument to its method entry point. The additional `InputStream` field `ctrlOutStream` in the `InvokeResponseEx` class provides an *out of band* pipe stream for the child process to write any special information back to the parent invoker process. This pipe stream could be dedicated, say, for the child process to write heartbeat/health-status and error information to the parent. The response `InputStream` field `inStream` is intended to be used pretty much as before - for the primary output results of the child process to be consumed by its parent invoker.

This feature will require the addition of a new annotation which will be associated to a new method entry point signature. The method signature will have two additional stream arguments like so:

```java
@ChildWorkerCommandEx(cmd="CDCETL", jvmArgs={"-Xms128m", "-Xmx324m"})
public static void doCdcEtlProcessing(String[] args, PrintStream rspStream, PrintStream ctrlOutStream, InputStream ctrlInStream)
```

One very useful thing that can be done with the `InputStream` argument `ctrlInStream` is that the parent process can write the serialized form of startup configuration to the child process, which it should deserialize before proceeding with its activity. This would avoid the use of a file for passing startup configuration state.

This proposed new API will be more complicated to program with which is why it's not been provided up to now. And its not that difficult to multiplex `out of band` information into the single response stream that is supported now. Either a file approach or Java shared memory can currently be used to convey configuration from the parent to the child. So there are certainly arguments for pushing back on this particular feature addition. 
